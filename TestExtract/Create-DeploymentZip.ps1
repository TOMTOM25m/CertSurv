# Certificate WebService - Deployment Package ZIP Creator
...
try {
    ...
    if (Test-Path $zipPath) {
        ...
        # ZIP-Inhalt anzeigen
        Write-Host "📦 Paket-Inhalt:" -ForegroundColor Cyan
        $zip = [System.IO.Compression.ZipFile]::OpenRead($zipPath)
        try {
            $zip.Entries | ForEach-Object {
                $indent = "   " + ("  " * ([char[]]$_.FullName | Where-Object { $_ -eq '/' -or $_ -eq '\' }).Count)
        ...
        # Deployment-Befehle für verschiedene Server
