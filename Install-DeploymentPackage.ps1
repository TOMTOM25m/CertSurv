...
.DESCRIPTION
    ...
    Unterstuetzt verschiedene Server-Typen mit angepassten Port-Konfigurationen.
...
.PARAMETER SkipFirewall
    Firewall-Konfiguration berspringen
...
Write-Verbose "Install-DeploymentPackage - PowerShell Version: $($PSVersion.ToString())"
...
#region Universal Web Request Functions (Regelwerk v9.4.0)
function Invoke-WebRequestUniversal {
    param(
        [string]$Uri,
    ...
    try {
        if ($IsPS7Plus -and $Uri.StartsWith("https")) {
            Write-Verbose "Using PowerShell 7.x with SkipCertificateCheck for HTTPS"
            return Invoke-WebRequest -Uri $Uri @Parameters -SkipCertificateCheck
        } else {
            Write-Verbose "Using PowerShell 5.1 compatible mode"
            if ($Uri.StartsWith("https")) {
                # Extend certificate validation for PS5.1 without disabling default rules
                ...
                [System.Net.ServicePointManager]::ServerCertificateValidationCallback = {
                    ...
                    $defaultResult = $true
                    if ($originalCallback) {
                        $defaultResult = $originalCallback.Invoke($sender, $certificate, $chain, $sslPolicyErrors)
                    }
                    # Extend: allow self-signed certificates for localhost only
                    if ($sslPolicyErrors -eq [System.Net.Security.SslPolicyErrors]::RemoteCertificateChainErrors -and $certificate.Subject -like "*CN=localhost*") {
                        return $true
                    }
                    return $defaultResult
                }
                try {
                    return Invoke-WebRequest -Uri $Uri @Parameters
                } finally {
        ...
        Write-Verbose "WebRequest failed: $_"
        throw
    }
}
#endregion
...
try {
    ...
    if ($deploymentConfig.ContainsKey($ServerType) -and $ServerType -ne "Custom") {
        ...
        Write-DeploymentLog "Konfiguration fr $($config.Description): HTTP=$HttpPort, HTTPS=$HttpsPort"
    }
    
    #  VORAUSSETZUNGEN PRFEN
    Write-DeploymentLog " Prfe Voraussetzungen..."
    
    # Administrator-Rechte prfen
    if (-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
        throw "Dieses Skript muss als Administrator ausgefhrt werden!"
    }
    
    # PowerShell-Version prfen
    if ($PSVersionTable.PSVersion.Major -lt 5) {
        throw "PowerShell 5.1 oder hher erforderlich! Aktuelle Version: $($PSVersionTable.PSVersion)"
    }
    
    # Port-Verfgbarkeit prfen
    ...
    #  IIS INSTALLATION
    Write-DeploymentLog " Installiere IIS-Features..."
    ...
    if (Test-Path $InstallPath) {
        Write-DeploymentLog "Lsche existierende Installation: $InstallPath"
    ...
    # Lsche existierende Site falls vorhanden
    ...
    if (-not $SkipFirewall) {
        ...
        Write-DeploymentLog "Firewall-Regeln fr Ports $HttpPort und $HttpsPort erstellt"
    }
    ...
    if ($TestInstallation) {
        ...
        # HTTP-Test
        try {
            $httpResponse = Invoke-WebRequestUniversal -Uri "http://localhost:$HttpPort/certificates.json" -Parameters @{UseBasicParsing=$true; TimeoutSec=10}
        ...
        # HTTPS-Test  
        try {
            $httpsResponse = Invoke-WebRequestUniversal -Uri "https://localhost:$HttpsPort/certificates.json" -Parameters @{UseBasicParsing=$true; TimeoutSec=10}
