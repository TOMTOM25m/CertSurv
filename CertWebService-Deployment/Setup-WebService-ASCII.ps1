...
<#
.SYNOPSIS
    Certificate WebService Setup Script - Regelwerk v9.5.0 Compliant
.DESCRIPTION
    Server Core und Standard Server kompatible Version mit vollstÃ¤ndiger Regelwerk-Compliance.
    Implementiert Robocopy-Standards und Versionierungs-Anforderungen.
.VERSION HISTORY
    1.4.0 - 2025-09-23 - Regelwerk v9.5.0 Compliance, Robocopy Integration, Version Display
...
.NOTES
    ...
    Version: v1.4.0
    Regelwerk: v9.5.0
    Usage: Als Administrator auf Zielserver ausfÃ¼hren
#>

#region Script Version Information (MANDATORY - Regelwerk v9.5.0)
$ScriptVersion = "1.4.0"
...
# Get server FQDN for endpoint display
try {
    $domain = (Get-WmiObject -Class Win32_ComputerSystem).Domain
    $hostname = $serverFQDN
    $serverFQDN = "$hostname.$domain"
    Write-Host " Server FQDN: $serverFQDN" -ForegroundColor Cyan
} catch {
    $serverFQDN = $serverFQDN
    Write-Host " Could not determine FQDN, using hostname: $serverFQDN" -ForegroundColor Yellow
}
    Write-Host "ðŸš€ Certificate WebService Setup Script v$ScriptVersion" -ForegroundColor Green
    ...
    Write-Host "ðŸ–¥ï¸ Server: $serverFQDN" -ForegroundColor Yellow
...
Write-Verbose "Compatibility Mode: $(if($IsPS7Plus){'PowerShell 7.x'}elseif($IsPS51){'PowerShell 5.1'}else{'PowerShell 5.x'})"
#endregion

#region File Operations Standards (MANDATORY - Regelwerk v9.5.0)
function Copy-FilesRobust {
    ...
    Write-Host "ðŸ“ Starting robust file copy: $Source -> $Destination" -ForegroundColor Yellow
    ...
    Write-Host "âœ… File copy completed successfully" -ForegroundColor Green
}
...
function Test-NetworkPath {
    ...
    try {
        ...
        if ($TestResult) {
            Write-Host "âœ… Network path accessible: $Path" -ForegroundColor Green
            return $true
        } else {
            Write-Host "âš ï¸ Network path not accessible: $Path" -ForegroundColor Yellow
            return $false
        }
    }
    catch {
        Write-Host "âŒ Network path test failed: $Path - $($_.Exception.Message)" -ForegroundColor Red
        return $false
    }
}
#endregion
...
# Display Script Info (MANDATORY - Regelwerk v9.5.0)
Show-ScriptInfo
...
try {
    ...
    if (-not $isAdmin) {
        Write-Error "âŒ Script must be run as Administrator!"
        exit 1
    }
    ...
    Write-Host "ðŸ“¦ Detecting server type and installing IIS..." -ForegroundColor Yellow
    ...
    if ($availableFeatures.Count -eq 0) {
        Write-Host "âš ï¸ No IIS features found - trying alternative method..." -ForegroundColor Yellow
        ...
        try {
            ...
            Write-Host "âœ… IIS installed via Windows Optional Features" -ForegroundColor Green
        }
        catch {
            Write-Host "âŒ Failed to install IIS via Optional Features" -ForegroundColor Red
            ...
            Write-Host "âœ… IIS installed via DISM" -ForegroundColor Green
        }
    }
    else {
        ...
        foreach ($feature in $iisFeatures) {
            ...
            if ($featureState -and $featureState.InstallState -ne "Installed") {
                ...
                try {
                    ...
                    Write-Host "âœ… $feature installed" -ForegroundColor Green
                }
                catch {
                    Write-Host "âš ï¸ Could not install $feature - continuing..." -ForegroundColor Yellow
                }
            }
        }
        
        Write-Host "âœ… IIS features installation completed" -ForegroundColor Green
    }
    
    # Step 3: Create WebService directory
    Write-Host "ðŸ“ Creating WebService directory..." -ForegroundColor Yellow
    ...
    Write-Host "âœ… WebService directory created: $webServicePath" -ForegroundColor Green
    
    # Step 4: Create certificate update script
    Write-Host "ðŸ“ Creating certificate update script..." -ForegroundColor Yellow
    
    $updateScript = @'
#requires -Version 5.1
# Certificate Data Update Script for WebService - Regelwerk v9.5.0 Compliant
# Version: 1.4.0 | Author: Flecki (Tom) Garnreiter
...
try {
    Write-Host "ðŸ”„ Updating certificate data on $serverFQDN..."
    ...
    Write-Host "âœ… Certificate data updated successfully. Found $($allCerts.Count) certificates."
}
catch {
...
'@
    ...
    Write-Host "âœ… Certificate update script created" -ForegroundColor Green
    
    # Step 5: Create API endpoints
    Write-Host "ðŸ”§ Creating API endpoints..." -ForegroundColor Yellow
...
'@
    ...
    Write-Host "ðŸŒ Configuring IIS..." -ForegroundColor Yellow
    ...
    try {
        ...
        try {
            ...
            Write-Host "âš ï¸ Could not create HTTPS binding - continuing with HTTP only" -ForegroundColor Yellow
        }
        
        $iisConfigured = $true
        Write-Host "âœ… IIS configured via WebAdministration module" -ForegroundColor Green
    }
    catch {
        Write-Host "âš ï¸ WebAdministration module not available, trying alternative..." -ForegroundColor Yellow
    }
        ...
        try {
            ...
            if (Test-Path $appcmd) {
                ...
                try {
                    ...
                    Write-Host "âš ï¸ Could not create HTTPS binding via appcmd" -ForegroundColor Yellow
                }
                
                $iisConfigured = $true
                Write-Host "âœ… IIS configured via appcmd.exe" -ForegroundColor Green
            }
        }
        catch {
            Write-Host "âš ï¸ appcmd.exe configuration failed" -ForegroundColor Yellow
        }
    }
...
<html>
<head>
    <title>Certificate WebService v1.4.0</title>
    <meta http-equiv="refresh" content="0; url=certificates.json">
</head>
<body>
    <h1>Certificate WebService v1.4.0</h1>
    <p>Regelwerk: v9.5.0 Compliant</p>
...
'@
        ...
        Write-Host "âœ… File-based web service created" -ForegroundColor Green
        $iisConfigured = $true
    }
    
    # Step 7: Configure Windows Firewall
    Write-Host "ðŸ”¥ Configuring firewall..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "âš ï¸ Firewall configuration failed - please configure manually" -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "âœ… Scheduled task created (runs at 06:00 and 18:00)" -ForegroundColor Green
    }
    catch {
        Write-Host "âš ï¸ Scheduled task creation failed - please create manually" -ForegroundColor Yellow
    }
    
    # Step 9: Run initial certificate scan
    Write-Host "ðŸ” Running initial certificate scan..." -ForegroundColor Yellow
    
    try {
        & PowerShell.exe -ExecutionPolicy Bypass -File $updateScriptPath
        Write-Host "âœ… Initial certificate scan completed" -ForegroundColor Green
    }
    catch {
        Write-Host "âš ï¸ Initial scan failed - will retry automatically" -ForegroundColor Yellow
    }
    
    # Step 10: Final verification
    Write-Host "ðŸŽ¯ Final verification..." -ForegroundColor Yellow
    ...
    if ((Test-Path $certificatesJson) -and (Test-Path $healthJson)) {
        Write-Host "âœ… WebService files verified" -ForegroundColor Green
        
        # Show certificate count
        try {
            $certData = Get-Content $certificatesJson | ConvertFrom-Json
            $certCount = $certData.Certificates.Count
            Write-Host "ðŸ“Š Found $certCount certificates" -ForegroundColor Cyan
        }
        catch {
            Write-Host "ðŸ“Š Certificate data created (count verification failed)" -ForegroundColor Cyan
        }
    }
    else {
        Write-Host "âš ï¸ Some WebService files missing - please run Test-WebService.ps1" -ForegroundColor Yellow
    }
    
    Write-Host ""
    Write-Host "ðŸŽ‰ Certificate WebService Setup Complete!" -ForegroundColor Green
    Write-Host "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”" -ForegroundColor Green
    Write-Host ""
    Write-Host "ðŸ“ WebService Location: $webServicePath" -ForegroundColor Cyan
    Write-Host "ðŸŒ HTTP Endpoint: http://$serverFQDN`:9080/certificates.json" -ForegroundColor Cyan
    Write-Host "ðŸ”’ HTTPS Endpoint: https://$serverFQDN`:9443/certificates.json" -ForegroundColor Cyan
    Write-Host "â¤ï¸ Health Check: http://$serverFQDN`:9080/health.json" -ForegroundColor Cyan
    Write-Host "ðŸ“Š Summary: http://$serverFQDN`:9080/summary.json" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "ðŸ”„ Automatic Updates: Daily at 06:00 and 18:00" -ForegroundColor Yellow
    Write-Host "ðŸ”§ Manual Update: $updateScriptPath" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "ðŸ§ª Test the installation with: Test-WebService.ps1" -ForegroundColor Magenta
    Write-Host "ðŸ“‹ Regelwerk: $RegelwerkVersion Compliant" -ForegroundColor Green
    Write-Host ""
    
}
catch {
    Write-Host "âŒ Setup failed: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "Check the error details above and retry as Administrator." -ForegroundColor Yellow
    exit 1
}

# --- End of script --- v1.4.0 ; Regelwerk: v9.5.0 ---
