...
.DESCRIPTION
    Server Core und Standard Server kompatible Version mit vollständiger Regelwerk-Compliance.
...
function Show-ScriptInfo {
    Write-Host "🚀 Certificate WebService Setup Script v$ScriptVersion" -ForegroundColor Green
    Write-Host "📅 Build: $BuildDate | Regelwerk: $RegelwerkVersion" -ForegroundColor Cyan
    Write-Host "👤 Author: $Author" -ForegroundColor Cyan
    Write-Host "🖥️ Server: $env:COMPUTERNAME" -ForegroundColor Yellow
...
function Copy-FilesRobust {
    ...
    Write-Host "📁 Starting robust file copy: $Source -> $Destination" -ForegroundColor Yellow
...
function Test-NetworkPath {
    ...
    try {
        ...
        if ($TestResult) {
            Write-Host "✅ Network path accessible: $Path" -ForegroundColor Green
            return $true
        } else {
            Write-Host "⚠️ Network path not accessible: $Path" -ForegroundColor Yellow
            return $false
        }
    }
    catch {
        Write-Host "❌ Network path test failed: $Path - $($_.Exception.Message)" -ForegroundColor Red
        return $false
    }
}
#endregion
...
'@
    ...
    Write-Host "📋 Regelwerk: $RegelwerkVersion Compliant" -ForegroundColor Green
    Write-Host ""
